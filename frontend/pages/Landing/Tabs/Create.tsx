...
import styles from "../../../styles/LandingCSS/Tabs/Create.module.css";
import {
  Type,
  Shapes,
  LayoutTemplate,
...
export default function Create({ templateImage, initialPlatformId, onBack }: CreateProps) {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const availableW = Math.max(320, rect.width - CANVAS_AREA_PADDING);
      const availableH = Math.max(240, rect.height - 96);
      const byWidth = Math.min(CANVAS_DISPLAY_W, availableW);
      const byHeight = Math.floor((availableH * activePlatform.w) / activePlatform.h);
      setCanvasDisplayW(Math.max(320, Math.min(CANVAS_DISPLAY_W, byWidth, byHeight)));
    };
  ...
  }, [activePlatform.h, activePlatform.w]);

  const canvasH = Math.round((canvasDisplayW * activePlatform.h) / activePlatform.w);
  ...
  const updateElement = (id: string, patch: Partial<CanvasElement>, commit = true) => {
  ...
  const downloadDesign = async () => {
    ...
    canvas.width = activePlatform.w;
    canvas.height = activePlatform.h;
    ...
    for (const el of sortedElements) {
      ...
      if (el.type === "text") {
        ...
        ctx.font = `${el.fontStyle} ${el.fontWeight} ${el.fontSize}px Inter, Arial, sans-serif`;
        ctx.textAlign = el.align;
        ctx.textBaseline = "top";

        const textX =
          el.align === "center" ? x + width / 2 : el.align === "right" ? x + width : x;
        ...
        lines.forEach((line, index) => {
          ...
          if (el.textDecoration === "underline") {
            ...
            const underlineY = lineY + el.fontSize + 3;
            const startX =
              el.align === "center"
      ...
      if (el.type === "shape") {
        ...
        if (el.shape === "rect") {
          ...
          if (el.stroke !== "none" && el.strokeWidth > 0) ctx.strokeRect(x, y, width, height);
        }
        ...
        if (el.shape === "circle") {
          ...
          ctx.fill();
          if (el.stroke !== "none" && el.strokeWidth > 0) ctx.stroke();
        }
    ...
    canvas.toBlob((blob) => {
      ...
      link.href = url;
      link.download = `story-${activePlatform.id}-${Date.now()}.png`;
      link.click();
  ...
  useEffect(() => {
    ...
    const onUp = () => { pushHistory(elementsRef.current); setDragging(null); };
  ...
  const renderElement = (el: CanvasElement) => {
    if (el.type === "text") {
      const s: CSSProperties = {
        ...
        minHeight: 20,
        color: el.color,
        fontSize: el.fontSize * (canvasDisplayW / activePlatform.w),
      ...
      return (
        <div
          ...
          onMouseDown={(e) => {
            e.stopPropagation();
            if (textEditId !== el.id) onMouseDown(e, el.id);
          }}
          onDoubleClick={(e) => {
            e.stopPropagation();
            setSelectedId(el.id);
          ...
          onInput={(e) => {
            ...
            updateElement(el.id, { text }, false);
          }}
          onFocus={(e) => {
            if (textEditId === el.id) {
    ...
    <div className={styles.root}>
      <header className={styles.topbar}>
        ...
        <div className={styles.topRight}>
          ...
          <button className={styles.downloadBtn} onClick={downloadDesign}>
      ...
      <div className={styles.workspace}>
        ...
        <main ref={canvasAreaRef} className={styles.canvasArea} onClick={() => setSelectedId(null)}>
          ...
          <div
            ...
            style={{ width: canvasDisplayW, height: canvasH }}
